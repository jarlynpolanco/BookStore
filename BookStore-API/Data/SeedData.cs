    ...
    public static class SeedData
    {
        ...
        private async static Task SeedUsers(UserManager<IdentityUser> userManager) 
        {
            if (await userManager.FindByEmailAsync("admin@bookstore.com") == null)
            {
                ...
                var result = await userManager.CreateAsync(user, "Administrator@1234");
                if (result.Succeeded) 
                {
            ...
            if (await userManager.FindByEmailAsync("customer1@gmail.com") == null)
            {
                ...
                var result = await userManager.CreateAsync(user, "Customer@1");
                if (result.Succeeded)
                {
            ...
            if (await userManager.FindByEmailAsync("customer2@gmail.com") == null)
            {
                ...
                var result = await userManager.CreateAsync(user, "Customer@2");
                if (result.Succeeded)
                {
                    await userManager.AddToRoleAsync(user, "Customer");
                }
